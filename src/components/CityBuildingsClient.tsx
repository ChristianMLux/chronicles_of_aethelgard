...
import { doc, onSnapshot, runTransaction, DocumentData } from "firebase/firestore";
...
export default function CityBuildingsClient({ cityId, userId }: { cityId: string, userId: string }) {
    ...
    async function upgrade(building: BuildingKey) {
        ...
        try {
            ...
            await runTransaction(db, async (trx) => {
                ...
                if (!canAfford(c.resources, cost)) throw new Error("Nicht genügend Ressourcen");
    ...
    return (
        <div className="p-6 flex flex-col gap-4">
            <Link className="underline" href={`/city/${cityId}`}>← Zur Stadt</Link>
                        ...
                        <div key={b} className="border rounded p-3 flex items-center justify-between">
                            <div onClick={() => setSelected(b)} className="cursor-pointer">
                                <div className="font-medium flex items-center gap-2">
                                    <Image src={BUILDING_ICONS[b]} width={18} height={18} alt={b} className="icon-tile" />
                                    {b} — L{level}
                                </div>
                            ...
                            <button className="border rounded px-3 py-1 disabled:opacity-50" onClick={() => upgrade(b)} disabled={!can || busy === b}>
                                {busy === b ? "..." : "Ausbauen"}
                            </button>
                        </div>
                    );
                ...
                {selected && (
                    <div className="flex flex-col gap-3">
                        <div className="flex items-center gap-3 mb-4">
                            <Image src={BUILDING_ICONS[selected]} width={48} height={48} alt={selected} className="icon-tile"/>
                            <div>
